 ...
 * Legacy callback usage still supported: confirm(msg, onOkFn)
 */
export function confirm(msg, onOk, okLabel = "Confirm", danger = true) {
  return new Promise(resolve => {
    ...
    const iconColor   = danger ? "#dc2626" : "#4f46e5";
    const iconBg      = danger ? "#fee2e2" : "#e0e7ff";
    const btnBg       = danger ? "#dc2626" : "#4f46e5";
    ...
    el.innerHTML = `
      <div style="background:#fff;border-radius:16px;padding:24px;max-width:360px;width:100%;
        ...
        <div style="width:44px;height:44px;background:${iconBg};border-radius:50%;
             display:flex;align-items:center;justify-content:center;margin:0 auto 12px;">
          <svg style="width:20px;height:20px;color:${iconColor};" fill="none"
               stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">${iconPath}</svg>
        </div>
        ...
        <div style="display:flex;gap:10px;">
          ...
          <button id="pvc-ok" style="flex:1;padding:10px;border:none;border-radius:10px;font-size:13.5px;
            font-weight:600;cursor:pointer;background:${btnBg};color:#fff;">${okLabel}</button>
        </div>
      </div>`;
    document.body.appendChild(el);
    const cleanup = (result) => { el.remove(); resolve(result); if (result && typeof onOk === "function") onOk(); };
    document.getElementById("pvc-cancel").onclick = () => cleanup(false);
    document.getElementById("pvc-ok").onclick     = () => cleanup(true);
