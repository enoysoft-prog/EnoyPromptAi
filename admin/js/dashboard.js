...
// ── Extra stats: submissions & coin transactions ──────────
export async function loadExtraStats() {
  try {
    const [subDocs, txDocs, refDocs] = await Promise.all([
      safeDocs("submissions"),
      safeDocs("coinTransactions"),
      safeDocs("referrals"),
    ]);

    const pending  = subDocs.filter(s => s.status === "pending").length;
    const approved = subDocs.filter(s => s.status === "approved").length;
    const totalCoinsAwarded = txDocs
      .filter(t => t.type === "earn")
      .reduce((s, t) => s + (t.amount || 0), 0);

    set("ds-sub-pending",  pending);
    set("ds-sub-approved", approved);
    set("ds-coins-total",  totalCoinsAwarded);
    set("ds-referrals",    refDocs.length);
  } catch {}
}
